...
j=j.filter(function(){return this.parentNode===g.node()}));var k=j.data(i,f),l=k.enter().insert(d).attr(e).style("opacity",sc),m=a.transition(k.exit()).style("opacity",sc).remove();
...
nb(this)})}function qb(){return++Pc}function rb(a,b){function c(b){return a.secondaries.filter(function(a,c){return c===b})}for(var d=0,e=0;e<b.secondaries.length;e++)b.secondaries[e]&&d++;a.secondaries.filter(function(a,b){return d>b}).style("flex","1"),a.secondaries.filter(function(a,b){return b>=d}).style("flex","0"),a.overlaySecondaries.filter(function(a,b){return d>b}).style("flex","1"),a.overlaySecondaries.filter(function(a,b){return b>=d}).style("flex","0");var f=parseInt(a.app.select(".head-row").style("height"),10);Qc||(f+=parseInt(a.app.select(".head-row").style("padding-top"),10)+parseInt(a.app.select(".head-row").style("padding-bottom"),10)+parseInt(a.app.select(".head-row").style("margin-bottom"),10),Qc=!0);var g=window.innerHeight-f;a.charts.style("height",g+"px"),b.xAxis.dimensionChanged(a.xAxis),b.navbar.dimensionChanged(a.navbar),b.primary.dimensionChanged(a.primary);for(var h=0;h<b.secondaries.length;h++)b.secondaries[h].option.dimensionChanged(c(h))}function sb(b,c){var d=a.max(b,function(a){return a.getTime()}),e=a.max(c,function(a){return a.date.getTime()});return d===e}function tb(a,b){var c=Array.isArray(b)?b:[b,b],d=a[1]-a[0];return[a[0]-d*c[0],a[1]+d*c[1]]}function ub(b,c,d){arguments.length<3&&(d=1);var e=Oc.util.extent().fields("date")(c),f=(e[1].getTime()-e[0].getTime())/1e3,g=b.map(function(a){return a.getTime()}),h=d*(a.max(g)-a.min(g))/1e3,i=f>h?[a.time.second.offset(e[1],-h),e[1]]:e;return i}function vb(b,c){var d=a.min(b,function(a){return a.getTime()}),e=a.max(b,function(a){return a.getTime()}),f=c.sort(function(a,b){return a.date-b.date}),g=a.bisector(function(a){return a.date}),h=c.slice(
// Pad and clamp the bisector values to ensure extents can be calculated
Math.max(0,g.left(f,d)-1),Math.min(g.right(f,e)+1,f.length));return h}function wb(b,c,d){var e=Oc.util.extent().fields("date")(c),f=b.map(function(a){return a.getTime()}),g=(a.max(f)-a.min(f))/1e3;if(d.getTime()<e[0]||d.getTime()>e[1])return[new Date(a.min(f)),new Date(a.max(f))];var h=[a.time.second.offset(d,-g/2),a.time.second.offset(d,g/2)],i=0;return h[1].getTime()>e[1].getTime()?i=(e[1].getTime()-h[1].getTime())/1e3:h[0].getTime()<e[0].getTime()&&(i=(e[0].getTime()-h[0].getTime())/1e3),[a.time.second.offset(h[0],i),a.time.second.offset(h[1],i)]}function xb(b){function c(a){
// Don't pan off sides
// Don't pan off sides
return a[0]>=0?-a[0]:a[1]<=0?-a[1]:0}function d(a){
// If zooming, and about to pan off screen, do nothing
return a[0]>0&&a[1]<0}function e(a){var b=j.translate()[0];b+=a,j.translate([b,0])}function f(){j.translate([0,0]),j.scale(1)}function g(a){var g=Oc.util.extent().fields("date")(a.datum().data);j.x(h).on("zoom",function(){var n=h(g[0]),o=h(g[1]),p=d([n,o-b]),q=c([n,o-b]);e(q);var r=1===j.scale(),s=1!==j.scale();if(r&&k||s&&l){var t=h.domain();p?t=g:s&&m&&(t=Sc.domain.moveToLatest(t,a.datum().data)),t[0].getTime()!==t[1].getTime()?i.zoom(t):
// Ensure the user can't zoom-in infinitely, causing the chart to fail to render
// #168, #411
f()}else f()}),a.call(j)}var h,i=a.dispatch("zoom"),j=a.behavior.zoom(),k=!0,l=!0,m=!0;return g.allowPan=function(a){return arguments.length?(k=a,g):k},g.allowZoom=function(a){return arguments.length?(l=a,g):l},g.trackingLatest=function(a){return arguments.length?(m=a,g):m},g.scale=function(a){return arguments.length?(h=a,g):h},a.rebind(g,i,"on"),g}function yb(){function b(b){b.each(function(f){var h=a.select(this).call(j),i=xb(c).scale(e).trackingLatest(g).on("zoom",function(a){d[Tc.viewChange](a)});h.select(".plot-area-container").datum({data:b.datum()}).call(i)})}var c,d=a.dispatch(Tc.viewChange),e=Oc.scale.dateTime(),f=a.scale.linear(),g=!0,h=60,i=Oc.series.multi(),j=Oc.chart.cartesian(e,f).plotArea(i).xTicks(0).yOrient("right").margin({top:0,left:0,bottom:0,right:h});return b.trackingLatest=function(a){return arguments.length?(g=a,b):g},a.rebind(b,d,"on"),a.rebind(b,i,"series","mapping","decorate"),a.rebind(b,j,"yTickValues","yTickFormat","yTicks","xDomain","yDomain"),b.dimensionChanged=function(a){c=parseInt(a.style("width"),10)-h},b}function zb(){function b(a){a.each(function(b){var c=Oc.util.extent().fields("volume").pad(.08)(b.data);c[0]<0&&(c[0]=0),e.yTickFormat(b.product.volumeFormat).trackingLatest(b.trackingLatest).xDomain(b.viewDomain).yDomain(c),a.datum(b.data).call(e)})}var c=a.dispatch(Tc.viewChange),d=Oc.series.bar().yValue(function(a){return a.volume}),e=yb().series([d]).yTicks(4).on(Tc.viewChange,function(a){c[Tc.viewChange](a)});return a.rebind(b,c,"on"),b.dimensionChanged=function(a){e.dimensionChanged(a)},b}function Ab(){function b(a){var b=a.datum();e(b.data),g.trackingLatest(b.trackingLatest).xDomain(b.viewDomain).yDomain([0,100]),a.datum(b.data).call(g)}var c=a.dispatch(Tc.viewChange),d=Oc.indicator.renderer.relativeStrengthIndex(),e=Oc.indicator.algorithm.relativeStrengthIndex(),f=[d.lowerValue(),50,d.upperValue()],g=yb().series([d]).yTickValues(f).on(Tc.viewChange,function(a){c[Tc.viewChange](a)});return a.rebind(b,c,"on"),b.dimensionChanged=function(a){g.dimensionChanged(a)},b}function Bb(){function b(a){var b=a.datum();f(b.data);var c=Oc.util.extent().fields("macd").symmetricalAbout(0).pad(.08)(b.data.map(function(a){return a.macd}));g.trackingLatest(b.trackingLatest).xDomain(b.viewDomain).yDomain(c),a.datum(b.data).call(g)}var c=a.dispatch(Tc.viewChange),d=Oc.annotation.line().value(0).label(""),e=Oc.indicator.renderer.macd(),f=Oc.indicator.algorithm.macd(),g=yb().series([d,e]).yTicks(5).mapping(function(a){return a===d?[0]:this}).decorate(function(a){a.enter().attr("class",function(a,b){return["multi zero","multi"][b]})}).on(Tc.viewChange,function(a){c[Tc.viewChange](a)});return a.rebind(b,c,"on"),b.dimensionChanged=function(a){g.dimensionChanged(a)},b}function Cb(){function b(a){var b=(d.ticks()[1]-d.ticks()[0])/1e3;b<a.seconds?e.ticks(a.d3TimeInterval.unit,a.d3TimeInterval.value):e.ticks(6)}function c(a){var c=a.datum();d.domain(c.viewDomain),b(c.period),a.call(e)}var d=Oc.scale.dateTime(),e=a.svg.axis().scale(d).orient("bottom");return c.dimensionChanged=function(a){d.range([0,parseInt(a.style("width"),10)])},c}function Db(a,b,c,d){return{displayString:a,// TODO: is 'displayName' better?
valueString:b,// TODO: is this an id?
option:c,// TODO: Ideally, remove.
isSelected:!1,icon:d}}function Eb(){var b=Oc.scale.dateTime(),c=a.scale.linear(),d=Oc.util.fractionalBarWidth(.75),e=function(a,b){return a.date},f=function(a,c){return b(e(a,c))},g=function(a){return a.low},h=function(a){return a.high},i=function(a,b){return a.close},j=Oc.svg.candlestick().x(function(a){return b(a.date)}).open(function(a){return c(a.open)}).high(function(a){return c(h(a))}).low(function(a){return c(g(a))}).close(function(a){return c(a.close)}),k=Oc.util.dataJoin().selector("path.up").element("path").attr("class","up"),l=Oc.util.dataJoin().selector("path.down").element("path").attr("class","down"),m=function(a){a.each(function(a){j.width(d(a.map(f)));var b=a.filter(function(a){return a.open<a.close}),c=a.filter(function(a){return a.open>=a.close});k(this,[b]).attr("d",j),l(this,[c]).attr("d",j)})};return m.xScale=function(a){return arguments.length?(b=a,m):b},m.yScale=function(a){return arguments.length?(c=a,m):c},m.xValue=function(a){return arguments.length?(e=a,m):e},m.yLowValue=function(a){return arguments.length?(g=a,m):g},m.yHighValue=function(a){return arguments.length?(h=a,m):h},m.yCloseValue=function(a){return arguments.length?(i=a,m):i},m.width=function(a){return d(a.map(f))},m}function Fb(a,b){var c=b/2;return[[0,0],[c,-c],[a,-c],[a,c],[c,c],[0,0]]}function Gb(a,b){for(var c=a.ticks.apply(a,[]),d=a.domain(),e=0;e<b.length;e++)b[e]>d[0]&&b[e]<d[1]&&c.push(b[e]);return c}function Hb(b,c,d){var e;switch(c.valueString){case"candlestick":case"ohlc":e=[c.option.yLowValue(),c.option.yHighValue()];break;case"line":case"point":e=c.option.yValue();break;case"area":e=c.option.y1Value();break;default:throw new Error("Main series given to chart does not have expected interface")}var f=Oc.util.extent().fields(e)(b);if(d.length){var g=d.map(function(a){return a.valueString}),h=-1!==g.indexOf("movingAverage"),i=-1!==g.indexOf("bollinger");if(i){var j=b.map(function(a){return a.bollingerBands}),k=Oc.util.extent().fields(["lower","upper"])(j);f[0]=a.min([k[0],f[0]]),f[1]=a.max([k[1],f[1]])}if(h){var l=Oc.util.extent().fields("movingAverage")(b);f[0]=a.min([l[0],f[0]]),f[1]=a.max([l[1],f[1]])}if(!h&&!i)throw new Error("Unexpected indicator type")}return f}function Ib(){function b(){var a=[q,i.option,r],b=n.map(function(a){return a.option});return a.concat(b,p)}function c(){switch(w.value(m),x.value(m),r.value(m),i.valueString){case"line":case"point":case"area":i.option.yValue(m)}}
// Call when what to display on the chart is modified (ie series, options)
function d(a){i=a.series,m=a.yValueAccessor.option,n=a.indicators,c(),s.series(b()),v.yTickFormat(a.product.priceFormat),a.selectorsChanged=!1}function e(a){var b=i.option.width(a);p.decorate(function(a){a.classed("band hidden-xs hidden-sm",!0),a.selectAll(".vertical > line").style("stroke-width",b)})}function f(a){a.classed("band",!1).classed("hidden-xs hidden-sm",!0).selectAll("line").style("stroke-width",null)}function g(a){"candlestick"===i.valueString||"ohlc"===i.valueString?e(a):p.decorate(f)}function h(b){var c=b.datum();c.selectorsChanged&&d(c),v.xDomain(c.viewDomain),p.snap(Oc.util.seriesPointSnapXOnly(i.option,c.data)),g(c.data),w(c.data),x(c.data);
// Scale y axis
var e=Sc.domain.filterDataInDateRange(v.xDomain(),c.data),f=Hb(e,i,n),h=Sc.domain.padYDomain(f,.04);v.yDomain(h);
...
// Redraw
v.plotArea(s),b.call(v);var r=xb(j).scale(t).trackingLatest(c.trackingLatest).on("zoom",function(a){l[Tc.viewChange](a)});b.select(".plot-area").call(r)}var i,j,k=60,l=a.dispatch(Tc.viewChange,Tc.crosshairChange),m=function(a){return a.close},n=[],o=[],p=Oc.tool.crosshair().xLabel("").yLabel("").on("trackingmove",function(a){a.length>0?l.crosshairChange(a[0].datum):l.crosshairChange(void 0)}).on("trackingend",function(){l.crosshairChange(void 0)});p.id=Sc.uid();var q=Oc.annotation.gridline().yTicks(5).xTicks(0),r=Oc.annotation.line().orient("horizontal").value(m).label("");r.id=Sc.uid();var s=Oc.series.multi().key(function(a){return a.id}).mapping(function(a){switch(a){case r:return[this.data[this.data.length-1]];case p:return o;default:return this.data}}),t=Oc.scale.dateTime(),u=a.scale.linear(),v=Oc.chart.cartesian(t,u).xTicks(0).yOrient("right").margin({top:0,left:0,bottom:0,right:k}),w=Oc.indicator.algorithm.movingAverage(),x=Oc.indicator.algorithm.bollingerBands();
// Call when the main layout is modified
return a.rebind(h,l,"on"),h.dimensionChanged=function(a){j=parseInt(a.style("width"),10)-k},h}function Jb(){function b(a,b){a.select(".plot-area").selectAll(".e, .w").classed("hidden",b)}function c(a){return a.extent()[0][0]-a.extent()[1][0]===0}function d(a){var d=a.datum();q.domain(d.viewDomain);var f=Sc.domain.filterDataInDateRange(Oc.util.extent().fields("date")(d.data),d.data),g=Oc.util.extent().fields(["low","high"])(f);p.xDomain(Oc.util.extent().fields("date")(d.data)).yDomain(g),t.on("brush",function(){var d=c(t);
// Hide the bar if the extent is empty
b(a,d),d||o[Tc.viewChange]([t.extent()[0][0],t.extent()[1][0]])}).on("brushend",function(){var e=c(t);b(a,!1),e&&o[Tc.viewChange](Sc.domain.centerOnDate(q.domain(),d.data,t.extent()[0][0]))}),p.plotArea(u),a.call(p);
// Allow to zoom using mouse, but disable panning
var h=xb(e).scale(q).trackingLatest(d.trackingLatest).allowPan(!1).on("zoom",function(a){o[Tc.viewChange](a)});a.select(".plot-area").call(h)}var e,f=100,g=40,h=f-g,i=2,j=i/2,k=h-j,l=k,m=j+l/2,n=2,o=a.dispatch(Tc.viewChange),p=Oc.chart.cartesian(Oc.scale.dateTime(),a.scale.linear()).yTicks(0).margin({bottom:g}),q=Oc.scale.dateTime(),r=Oc.series.area().yValue(function(a){return a.close}),s=Oc.series.line().yValue(function(a){return a.close}),t=a.svg.brush(),u=Oc.series.multi().series([r,s,t]).decorate(function(a){var b=a.enter();a.select(".extent").attr("height",k).attr("y",i/2),
...
var c=b.selectAll(".e, .w");c.append("circle").attr("cy",m).attr("r",7).attr("class","outer-handle"),c.append("circle").attr("cy",m).attr("r",4).attr("class","inner-handle")}).mapping(function(a){return a!==t?this.data:void t.extent([[q.domain()[0],p.yDomain()[0]],[q.domain()[1],p.yDomain()[1]]])});return a.rebind(d,o,"on"),d.dimensionChanged=function(a){e=parseInt(a.style("width"),10),q.range([0,e])},d}function Kb(){function b(b){b.each(function(b){var h=a.select(this);if(c=b.product.priceFormat,d=b.product.volumeFormat,e=b.period.timeFormat,null==b.data||b.data!==f){f=b.data;var i=h.selectAll("span").data(g);i.enter().append("span").attr("class",function(a,b){return b%2===0?"legendLabel":"legendValue"}),i.text(function(a,c){var d="";return c%2===0?a:b.data?a(b.data):d})}})}var c,d,e,f,g=["T",function(a){return e(a.date)},"O",function(a){return c(a.open)},"H",function(a){return c(a.high)},"L",function(a){return c(a.low)},"C",function(a){return c(a.close)},"V",function(a){return d(a.volume)}];return b}function Lb(a,b){return{historic:a,streaming:b}}function Mb(b){return{id:b.id,display:b.display||"Unspecified Product",priceFormat:a.format(b.priceFormat||".2f"),volumeFormat:a.format(b.volumeFormat||".2f"),periods:b.periods||[],source:b.source}}function Nb(b){return b=b||{},{display:b.display||"1 day",seconds:b.seconds||86400,d3TimeInterval:b.d3TimeInterval||{unit:a.time.day,value:1},timeFormat:a.time.format(b.timeFormat||"%b %d")}}function Ob(a){return{viewDomain:[],period:a}}function Pb(a){return{data:[],viewDomain:[],trackingLatest:!0,product:a}}function Qb(a){var b={data:[],trackingLatest:!0,viewDomain:[],selectorsChanged:!0},c=a;Object.defineProperty(b,"product",{get:function(){return c},set:function(a){c=a,b.selectorsChanged=!0}});var d=Eb();d.id=Sc.uid();var e=Db("Candlestick","candlestick",d);Object.defineProperty(b,"series",{get:function(){return e},set:function(a){e=a,b.selectorsChanged=!0}});var f={option:function(a){return a.close}};Object.defineProperty(b,"yValueAccessor",{get:function(){return f},set:function(a){f=a,b.selectorsChanged=!0}});var g=[];return Object.defineProperty(b,"indicators",{get:function(){return g},set:function(a){g=a,b.selectorsChanged=!0}}),b}function Rb(){return{trackingLatest:!0}}function Sb(){return{data:[],viewDomain:[],trackingLatest:!0}}function Tb(a,b){return{data:void 0,product:a,period:b}}function Ub(){return{primaryIndicators:[],secondaryIndicators:[]}}
...
function Wb(a){return{displayString:a.display,option:a}}function Xb(a,b,c){return{productConfig:{title:null,careted:!0,listIcons:!1,icon:!1},mobilePeriodConfig:{title:null,careted:!1,listIcons:!1,icon:!1},products:a,selectedProduct:b,selectedPeriod:c}}function Yb(){var a=Eb();a.id=Sc.uid();var b=Db("Candlestick","candlestick",a,"sc-icon-candlestick-series");b.isSelected=!0;var c=Oc.series.ohlc();c.id=Sc.uid();var d=Oc.series.line();d.id=Sc.uid();var e=Oc.series.point();e.id=Sc.uid();var f=Oc.series.area();return f.id=Sc.uid(),{config:{title:null,careted:!1,listIcons:!0,icon:!0},options:[b,Db("OHLC","ohlc",c,"sc-icon-ohlc-series"),Db("Line","line",d,"sc-icon-line-series"),Db("Point","point",e,"sc-icon-point-series"),Db("Area","area",f,"sc-icon-area-series")]}}function Zb(){return{config:{title:"Add Indicator",careted:!1,listIcons:!0,icon:!1},indicatorOptions:$c,secondaryChartOptions:_c}}function $b(){
// TODO: Instantiate series/indicator components outside of menu model?
return{seriesSelector:Yb(),indicatorSelector:Zb()}}function _b(){function b(b){var g=a.time.second.offset(e.end(),-d*c);e.start(g).collapse(f.get(c)),e(b)}var c,d,e=Oc.data.feed.quandl(),f=a.map();return f.set(86400,"daily"),f.set(604800,"weekly"),b.candles=function(a){return arguments.length?(d=a,b):d},b.granularity=function(a){if(!arguments.length)return c;if(!f.has(a))throw new Error("Granularity of "+a+" is not supported.");return c=a,b},Oc.util.rebind(b,e,{end:"end",product:"dataset"}),b}
// https://docs.exchange.coinbase.com/#websocket-feed
function ac(){var b,c="BTC-USD",d=a.dispatch("open","close","error","message"),e="match",f=function(a,f){a=a||"wss://ws-feed.exchange.coinbase.com",f=f||{type:"subscribe",product_id:c},b=new WebSocket(a),b.onopen=function(a){b.send(JSON.stringify(f)),d.open(a)},b.onerror=function(a){d.error(a)},b.onclose=function(a){d.close(a)},b.onmessage=function(a){var b=JSON.parse(a.data);b.type===e?d.message(b):"error"===b.type&&d.error(b)}};return a.rebind(f,d,"on"),f.close=function(){b&&b.close()},f.messageType=function(a){return arguments.length?(e=a,f):e},f.product=function(a){return arguments.length?(c=a,f):c},f}function bc(){function b(b){var e=a.time.second.offset(d.end(),-c*d.granularity());d.start(e),d(b)}var c,d=Oc.data.feed.coinbase();return b.candles=function(a){return arguments.length?(c=a,b):c},a.rebind(b,d,"end","granularity","product"),b}function cc(){var a,b,c,d=Oc.data.random.financial(),e=[86400],f=null,g=function(c){b.setHours(0,0,0,0);var e=864e5;d.startDate(new Date(b-(a-1)*e));var f=d(a);c(null,f)};return g.candles=function(b){return arguments.length?(a=b,g):a},g.end=function(a){return arguments.length?(b=a,g):b},g.granularity=function(a){if(!arguments.length)return c;if(-1===e.indexOf(a))throw new Error("Granularity of "+a+" is not supported. Random Financial Data Generator only supports daily data.");return c=a,g},g.product=function(a){if(!arguments.length)return g;if(null!==a)throw new Error("Random Financial Data Generator does not support products.");return f=a,g},g}function dc(){function b(b){b.each(function(b){var d=a.select(this),e=d.selectAll("div").data(b.selectedIndicators,function(a){return a.valueString}),f=e.enter().append("div").attr("class","edit-indicator");f.append("span").attr("class","icon sc-icon-delete").on("click",c.indicatorChange),f.append("span").attr("class","indicator-label").text(function(a){return a.displayString}),e.exit().remove()})}var c=a.dispatch(Tc.indicatorChange);return a.rebind(b,c,"on"),b}function ec(){var b=a.dispatch(Tc.primaryChartIndicatorChange,Tc.secondaryChartChange),c=dc().on(Tc.indicatorChange,b[Tc.primaryChartIndicatorChange]),d=dc().on(Tc.indicatorChange,b[Tc.secondaryChartChange]),e=function(b){b.each(function(b){var e=a.select(this);e.select("#overlay-primary-container .edit-indicator-container").datum({selectedIndicators:b.primaryIndicators}).call(c),e.selectAll(".overlay-secondary-container").each(function(c,e){var f=a.select(this),g=b.secondaryIndicators[e]?[b.secondaryIndicators[e]]:[];f.select(".edit-indicator-container").datum({selectedIndicators:g}).call(d)})})};return a.rebind(e,b,"on"),e}function fc(){function b(a){var b=a.datum(),d=a.selectAll("g").data([b]);d.enter().append("g").attr("class","reset-button").on("click",function(){c[Tc.resetToLatest]()}).append("path").attr("d","M1.5 1.5h13.438L23 20.218 14.937 38H1.5l9.406-17.782L1.5 1.5z"),d.classed("active",!b.trackingLatest)}var c=a.dispatch(Tc.resetToLatest);return a.rebind(b,c,"on"),b}function gc(){function b(a){var b=a.datum(),h=b.selectedIndex||0,i=b.config,j=d(a,[b.options]);if(i.icon){var k=j.selectAll(".icon").data([0]);k.enter().append("span"),k.attr("class","icon "+b.options[h].icon)}else j.select(".icon").remove(),j.text(function(){return i.title||b.options[h].displayString});e(j,i.careted?[0]:[]);var l=f(a,[b.options]),m=g(l,b.options),n=m.enter().on("click",c.optionChange).append("a").attr("href","#");n.append("span").attr("class","icon"),n.append("span").attr("class","name"),m.selectAll(".icon").attr("class",function(a){return"icon "+a.icon}),m.selectAll(".name").text(function(a){return a.displayString})}var c=a.dispatch("optionChange"),d=Oc.util.dataJoin().selector("button").element("button").attr({"class":"dropdown-toggle",type:"button","data-toggle":"dropdown"}),e=Oc.util.dataJoin().selector(".caret").element("span").attr("class","caret"),f=Oc.util.dataJoin().selector("ul").element("ul").attr("class","dropdown-menu"),g=Oc.util.dataJoin().selector("li").element("li").key(function(a){return a.displayString});return a.rebind(b,c,"on"),b}function hc(){var b=a.dispatch(Tc.primaryChartSeriesChange,Tc.primaryChartIndicatorChange,Tc.secondaryChartChange),c=gc().on("optionChange",b[Tc.primaryChartSeriesChange]),d=gc().on("optionChange",function(a){0===a.valueString.indexOf("secondary")?b[Tc.secondaryChartChange](a):b[Tc.primaryChartIndicatorChange](a)}),e=function(b){b.each(function(b){var e=a.select(this),f=b.seriesSelector.options.map(function(a){return a.isSelected}).indexOf(!0);e.select("#series-dropdown").datum({config:b.seriesSelector.config,options:b.seriesSelector.options,selectedIndex:f}).call(c);var g=b.indicatorSelector.indicatorOptions.concat(b.indicatorSelector.secondaryChartOptions),h=g.map(function(a,b){return a.isSelected?b:null}).filter(function(a){return a});e.select("#indicator-dropdown").datum({config:b.indicatorSelector.config,options:g,selected:h}).call(d)})};return a.rebind(e,b,"on"),e}function ic(){function b(a){var b=a.datum().selectedIndex||0,e=d(a,[a.datum().options]);e.enter().append("ul");var f=e.selectAll("li").data(Oc.util.fn.identity);f.enter().append("li").append("a").attr("href","#").on("click",c.tabClick),f.classed("active",function(a,c){return c===b}).select("a").text(function(a){return a.displayString}),f.exit().remove()}var c=a.dispatch("tabClick"),d=Oc.util.dataJoin().selector("ul").element("ul");return a.rebind(b,c,"on"),b}function jc(){var b=a.dispatch(Tc.dataProductChange,Tc.dataPeriodChange,Tc.clearAllPrimaryChartIndicatorsAndSecondaryCharts),c=gc().on("optionChange",b[Tc.dataProductChange]),d=ic().on("tabClick",b[Tc.dataPeriodChange]),e=gc().on("optionChange",b[Tc.dataPeriodChange]),f=function(f){f.each(function(g){var h=a.select(this),i=g.products;h.select("#product-dropdown").datum({config:g.productConfig,options:i.map(Vb),selectedIndex:i.indexOf(g.selectedProduct)}).call(c);var j=g.selectedProduct.periods;h.select("#period-selector").classed("hidden",j.length<=1).datum({options:j.map(Wb),selectedIndex:j.indexOf(g.selectedPeriod)}).call(d),h.select("#mobile-period-selector").classed("hidden",j.length<=1).datum({config:g.mobilePeriodConfig,options:j.map(Wb),selectedIndex:j.indexOf(g.selectedPeriod)}).call(e),h.select("#clear-indicators").on("click",b[Tc.clearAllPrimaryChartIndicatorsAndSecondaryCharts]),f.select("#toggle-button").on("click",function(){b[Tc.toggleSlideout]()})})};return a.rebind(f,b,"on"),f}function kc(){function a(a){var c=++b;return function(d,e){b>c||a(d,e)}}var b=0;return a.invalidateCallback=function(){return b++,a},a}function lc(){function b(a){var b=1e3*f;return new Date(Math.floor(a.getTime()/b)*b)}var c=function(a){return a.date},d=function(a){return Number(a.volume)},e=function(a){return Number(a.price)},f=60,g=function(f,g){var h=b(c(g)),i=e(g),j=d(g),k=a.bisector(function(a){return a.date}).left,l=f.filter(function(a){return a.date.getTime()===h.getTime()})[0];l?(l.high=Math.max(i,l.high),l.low=Math.min(i,l.low),l.close=i,l.volume+=j):f.splice(k(f,h),0,{date:h,open:i,high:i,low:i,close:i,volume:j})};return g.granularity=function(a){return arguments.length?(f=a,g):f},g.price=function(a){return arguments.length?(e=a,g):e},g.volume=function(a){return arguments.length?(d=a,g):d},g.date=function(a){return arguments.length?(c=a,g):c},g}function mc(){function b(){f&&f.close(),k=[],i.invalidateCallback()}function c(a){return a.sort(function(a,b){return a.date-b.date})}function d(a,d){b(),2===arguments.length&&(e=d.source.historic,e.product(d.id),f=d.source.streaming,null!=f&&f.product(d.id));var l=new Date;e.end(l).candles(j).granularity(a),h.granularity(a),e(i(function(a,b){a?g[Tc.historicFeedError](a):(k=c(b),g[Tc.historicDataLoaded](k))})),null!=f&&(f.on("message",function(a){h(k,a),g[Tc.newTrade](k)}).on("error",function(a){
// TODO: The 'close' event is potentially more useful for error info.
g[Tc.streamingFeedError](a)}),f())}var e,f,g=a.dispatch(Tc.historicDataLoaded,Tc.historicFeedError,Tc.newTrade,Tc.streamingFeedError),h=lc().date(function(a){return new Date(a.time)}).volume(function(a){return Number(a.size)}),i=kc(),j=200,k=[];return a.rebind(d,g,"on"),d}function nc(b){a.json("https://api.exchange.coinbase.com/products",function(a,c){return a?void b(a):void b(a,c)})}function oc(a,b,c,d){var e=a.map(function(a){return d.has(a.id)?{id:a.id,display:a.id,periods:d.get(a.id),source:b}:{id:a.id,display:a.id,periods:c,source:b}});return e.map(bd.data.product)}function pc(){function b(){ba&&H.suspendLayout(!1),H.primary.datum(S).call(_.primary),H.legend.datum(Z).call(_.legend),H.secondaries.datum(T).filter(function(a,b){return b<_.secondaries.length}).each(function(b,c){a.select(this).attr("class","secondary-container "+_.secondaries[c].valueString).call(_.secondaries[c].option)}),H.xAxis.datum(V).call(_.xAxis),H.navbar.datum(W).call(_.navbar),H.app.select("#navbar-reset").datum(X).call(B),H.app.select(".head-menu").datum(Y).call(A),H.app.select("#selectors").datum(U).call(C),H.overlay.datum($).call(G),ba&&(H.suspendLayout(!0),ba=!1)}function c(){ba=!0,Sc.layout(H,_)}function d(){a.select(window).on("resize",function(){c(),aa()})}function e(a){var b=[a[0],a[1]];S.viewDomain=b,T.viewDomain=b,V.viewDomain=b,W.viewDomain=b;var c=Sc.domain.trackingLatestData(S.viewDomain,S.data);S.trackingLatest=c,T.trackingLatest=c,W.trackingLatest=c,X.trackingLatest=c,aa()}function f(a){a.isSelected=!a.isSelected,x(),aa()}function g(a){a.isSelected=!a.isSelected,y(),aa()}function h(a){Z.data=a,aa()}function i(){var a=S.data,b=Oc.util.extent().fields("date")(a),c=Sc.domain.moveToLatest(b,a,.2);e(c)}function j(a){E.select("#loading-message").classed("hidden",!a),E.select("#charts").classed("hidden",a)}function k(a){S.data=a,T.data=a,W.data=a}function l(a){Y.selectedProduct=a,S.product=a,T.product=a,Z.product=a}function m(a){Y.selectedPeriod=a,V.period=a,Z.period=a}function n(){return Vc.primary().on(Tc.crosshairChange,h).on(Tc.viewChange,e)}function o(){return Vc.nav().on(Tc.viewChange,e)}function p(){return cd.navigationReset().on(Tc.resetToLatest,i)}function q(){return mc().on(Tc.newTrade,function(a){if(k(a),S.trackingLatest){var b=Sc.domain.moveToLatest(S.viewDomain,S.data);e(b)}}).on(Tc.historicDataLoaded,function(a){j(!1),k(a),Z.data=null,i(),c()}).on(Tc.historicFeedError,function(a){console.log("Error getting historic data: "+a)}).on(Tc.streamingFeedError,function(a){console.log("Error loading data from websocket: "+a)})}function r(a){return cd.head().on(Tc.dataProductChange,function(b){j(!0),l(b.option),m(b.option.periods[0]),a(b.option.periods[0].seconds,b.option),aa()}).on(Tc.dataPeriodChange,function(b){j(!0),m(b.option),a(b.option.seconds),aa()}).on(Tc.clearAllPrimaryChartIndicatorsAndSecondaryCharts,function(){S.indicators.forEach(t),_.secondaries.forEach(t),x(),y(),aa()})}function s(a,b){b.forEach(function(a){a.isSelected=!1}),a.isSelected=!0}function t(a){a.isSelected=!1}function u(){nc(v)}function v(b,c){if(b)console.log("Error getting Coinbase products: "+b);else{var d=[K,J],e=a.map();e.set("BTC-USD",[M,L,K,J]);var f=oc(c,O,d,e);Y.products=Y.products.concat(f),aa()}}function w(){return cd.selectors().on(Tc.primaryChartSeriesChange,function(a){S.series=a,s(a,U.seriesSelector.options),aa()}).on(Tc.primaryChartIndicatorChange,f).on(Tc.secondaryChartChange,g)}function x(){S.indicators=U.indicatorSelector.indicatorOptions.filter(function(a){return a.isSelected}),$.primaryIndicators=S.indicators}function y(){_.secondaries=U.indicatorSelector.secondaryChartOptions.filter(function(a){return a.isSelected}),
// TODO: This doesn't seem to be a concern of menu.
_.secondaries.forEach(function(a){a.option.on(Tc.viewChange,e)}),$.secondaryIndicators=_.secondaries,
// TODO: Remove .remove! (could a secondary chart group component manage this?).
H.secondaries.selectAll("*").remove(),c()}function z(){return cd.overlay().on(Tc.primaryChartIndicatorChange,f).on(Tc.secondaryChartChange,g)}var A,B,C,D={},E=a.select("#app-container"),F=E.select("#charts-container"),G=E.select("#overlay"),H={app:E,charts:F,primary:F.select("#primary-container"),secondaries:F.selectAll(".secondary-container"),xAxis:F.select("#x-axis-container"),navbar:F.select("#navbar-container"),overlay:G,overlaySecondaries:G.selectAll(".overlay-secondary-container"),legend:E.select("#legend"),suspendLayout:function(a){var b=this;Object.keys(b).forEach(function(c){"function"!=typeof b[c]&&b[c].layoutSuspended(a)})}},I=bd.data.period({display:"Weekly",seconds:604800,d3TimeInterval:{unit:a.time.week,value:1},timeFormat:"%b %d"}),J=bd.data.period({display:"Daily",seconds:86400,d3TimeInterval:{unit:a.time.day,value:1},timeFormat:"%b %d"}),K=bd.data.period({display:"1 Hr",seconds:3600,d3TimeInterval:{unit:a.time.hour,value:1},timeFormat:"%b %d %Hh"}),L=bd.data.period({display:"5 Min",seconds:300,d3TimeInterval:{unit:a.time.minute,value:5},timeFormat:"%H:%M"}),M=bd.data.period({display:"1 Min",seconds:60,d3TimeInterval:{unit:a.time.minute,value:1},timeFormat:"%H:%M"}),N=bd.data.source(cc()),O=bd.data.source(bc(),ac()),P=bd.data.source(_b()),Q=bd.data.product({id:null,display:"Data Generator",volumeFormat:".3s",periods:[J],source:N}),R=bd.data.product({id:"GOOG",display:"GOOG",volumeFormat:".3s",periods:[J,I],source:P}),S=bd.chart.primary(Q),T=bd.chart.secondary(Q),U=bd.menu.selectors(),V=bd.chart.xAxis(J),W=bd.chart.nav(),X=bd.chart.navigationReset(),Y=bd.menu.head([Q,R],Q,J),Z=bd.chart.legend(Q,J),$=bd.menu.overlay(),_={primary:void 0,secondaries:[],xAxis:Vc.xAxis(),navbar:void 0,legend:Vc.legend()},aa=Oc.util.render(b),ba=!0;return D.run=function(){_.primary=n(),_.navbar=o();var a=q();A=r(a),B=p(),C=w(),G=z(),c(),d(),a(Q.periods[0].seconds,Q),u()},D}a="default"in a?a["default"]:a;var qc={boundingBox:p,greedy:o,local:n},rc=Object.freeze({context:q,identity:r,index:s,noop:t}),sc=1e-6,tc=Object.freeze({isOrdinal:v,range:w,setRange:x}),uc={rectangles:z,strategy:qc},vc={dataJoin:u,expandRect:J,extent:I,fn:rc,minimum:e,fractionalBarWidth:H,innerDimensions:G,rebind:b,rebindAll:d,scale:tc,noSnap:B,pointSnap:C,seriesPointSnap:D,seriesPointSnapXOnly:E,seriesPointSnapYOnly:F,render:A,arrayFunctor:y},wc={crosshair:R,fibonacciFan:M,container:L,measure:K},xc={axis:W,bar:V,candlestick:U,ohlc:T,errorBar:S},yc={waterfall:X},zc={area:ha,bar:fa,stack:da,line:ea},Ac={area:ga,axis:la,bar:Y,candlestick:ka,cycle:ja,line:ca,multi:O,ohlc:ia,point:Q,stacked:zc,groupedBar:ba,xyBase:P,ohlcBase:aa,errorBar:_,waterfall:Z,algorithm:yc};pa.tickTransformer=na;var Bc={discontinuity:{identity:ma,skipWeekends:qa},dateTime:pa},Cc={bollingerBands:xa,macd:wa,relativeStrengthIndex:va,stochasticOscillator:ua,forceIndex:ta,envelope:sa,elderRay:ra},Dc={bollingerBands:Ra,exponentialMovingAverage:ya,macd:Na,percentageChange:Qa,relativeStrengthIndex:Ka,stochasticOscillator:Ia,slidingWindow:Aa,undefinedInputAdapter:Ea,forceIndex:Ga,envelope:Da,elderRay:za},Ec={bollingerBands:Sa,calculator:Dc,exponentialMovingAverage:Pa,macd:Oa,merge:Ba,movingAverage:Ma,relativeStrengthIndex:La,stochasticOscillator:Ja,forceIndex:Ha,envelope:Fa,elderRay:Ca},Fc={algorithm:Ec,renderer:Cc},Gc={modeMedian:Wa,largestTriangleThreeBucket:Va,largestTriangleOneBucket:Ua,bucket:Ta},Hc={filter:{skipWeekends:$a},financial:Za,walk:Ya},Ic={coinbase:ab,quandl:_a},Jc={feed:Ic,random:Hc,spread:Xa,sampler:Gc};/**
...
sXML="<svg xmlns='http://www.w3.org/2000/svg'>"+a+"</svg>";for(var c=b.parseFromString(sXML,"text/xml").documentElement,d=c.firstChild;d;)this.appendChild(this.ownerDocument.importNode(d,!0)),d=d.nextSibling}catch(e){throw new Error("Error parsing XML string")}}})}}((1,eval)("this").SVGElement);var Kc={cartesian:eb,sparkline:db,tooltip:cb,smallMultiples:bb},Lc={band:gb,gridline:fb,line:N},Mc=function(a){var b=a.exports;
...
var Nc="development",Oc={annotation:Lc,chart:Kc,data:Jc,indicator:Fc,scale:Bc,series:Ac,svg:xc,tool:wc,util:vc,version:Nc,layout:uc},Pc=0,Qc=!1,Rc={centerOnDate:wb,filterDataInDateRange:vb,moveToLatest:ub,padYDomain:tb,trackingLatestData:sb},Sc={domain:Rc,layout:rb,uid:qb},Tc={crosshairChange:"crosshairChange",viewChange:"viewChange",newTrade:"newTrade",historicDataLoaded:"historicDataLoaded",historicFeedError:"historicFeedError",streamingFeedError:"streamingFeedError",dataProductChange:"dataProductChange",dataPeriodChange:"dataPeriodChange",resetToLatest:"resetToLatest",clearAllPrimaryChartIndicatorsAndSecondaryCharts:"clearAllPrimaryChartIndicatorsAndSecondaryCharts",primaryChartSeriesChange:"primaryChartSeriesChange",primaryChartYValueAccessorChange:"primaryChartYValueAccessorChange",primaryChartIndicatorChange:"primaryChartIndicatorChange",secondaryChartChange:"secondaryChartChange",indicatorChange:"indicatorChange"},Uc={base:yb,macd:Bb,rsi:Ab,volume:zb},Vc={legend:Kb,nav:Jb,primary:Ib,xAxis:Cb,secondary:Uc},Wc={period:Nb,product:Mb,source:Lb},Xc={legend:Tb,nav:Sb,navigationReset:Rb,primary:Qb,secondary:Pb,xAxis:Ob},Yc=Oc.series.line().decorate(function(a){a.enter().classed("movingAverage",!0)}).yValue(function(a){return a.movingAverage});Yc.id=Sc.uid();var Zc=Oc.indicator.renderer.bollingerBands();Zc.id=Sc.uid();var $c=[Db("Moving Average","movingAverage",Yc,"sc-icon-moving-average-indicator"),Db("Bollinger Bands","bollinger",Zc,"sc-icon-bollinger-bands-indicator")],_c=[Db("Relative Strength Index","secondary-rsi",Uc.rsi(),"sc-icon-rsi-indicator"),Db("MACD","secondary-macd",Uc.macd(),"sc-icon-macd-indicator"),Db("Volume","secondary-volume",Uc.volume(),"sc-icon-bar-series")],ad={selectors:$b,head:Xb,periodAdaptor:Wb,productAdaptor:Vb,overlay:Ub},bd={menu:ad,chart:Xc,data:Wc},cd={head:jc,selectors:hc,navigationReset:fc,overlay:ec};pc().run()});